...
- **SSH/ターミナル方向キー互換性**: `read_menu_key()` を適切な ANSI escape sequence パーサーで書き直しました。`select.select` と短いタイムアウトを廃止し、`termios` VTIME と `os.read(fd, 1)` を使用して SSH 遅延によるエスケープシーケンスの分割問題を解消。方向キー、Home、End が SSH セッションで安定して動作します。
