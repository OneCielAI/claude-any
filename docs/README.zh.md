...
> - **本地 provider 浏览器聊天** — router 提供 `/ca/web/chat`，通过与 Claude Code 相同的 `/v1/messages` 路径向当前 provider 发送独立浏览器会话；它不会附着到正在运行的 Claude Code 终端会话。
...
3. **Router provider 浏览器聊天和可选 Anthropic 路由** — `/ca/web/chat` 提供向当前 provider 发送独立 `/v1/messages` 会话的本地 router chat UI；需要 SSE/channel/observability 时，Anthropic 也可以选择走 Claude Any router。
...
不会自动连接到正在运行的 Claude Code 终端 transcript 或任务状态。它不会自动创建 Cloudflare
