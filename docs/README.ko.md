...
3. **모델 capability override + 다중 API 키 검증** — `claude_code_supported_capabilities=effort,xhigh_effort,...` 옵션으로 provider/model별 Claude Code capability를 명시할 수 있습니다. Claude Opus/Sonnet 계열 ID는 가능한 범위에서 자동 추론합니다. provider에 여러 API 키가 설정되어 있으면 compatibility test가 각 키를 독립적으로 text probe 하여 죽은 키를 실행 전에 잡습니다.
