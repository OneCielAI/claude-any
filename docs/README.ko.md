...
> - **로컬 provider 브라우저 채팅** — router가 `/ca/web/chat`을 제공하며, Claude Code와 같은 `/v1/messages` 경로로 현재 선택된 provider에 독립 text-only 브라우저 대화를 보냅니다. 실행 중인 Claude Code 터미널 세션이나 tool executor에 붙는 UI는 아닙니다.
...
3. **Router provider 브라우저 채팅과 선택형 Anthropic 라우팅** — `/ca/web/chat`으로 현재 provider에 독립 text-only `/v1/messages` 대화를 보내는 로컬 router 채팅 화면을 제공하고, Anthropic도 필요할 때 Claude Any router를 경유해 SSE, 채널, 관측 기능을 사용할 수 있습니다.
...
transcript나 작업 상태에는 자동으로 연결되지 않습니다. 이 화면은 text-only입니다.
Claude Code 도구, MCP 도구, shell, 파일시스템 접근은 사용할 수 없습니다.
